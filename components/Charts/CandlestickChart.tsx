...
export function CandlestickChart({ symbol, height = 500, showVolume = true, indicators = [] }: ChartProps) {
  ...
  const updateIndicatorSeries = (config: IndicatorConfig, seriesObj: any, allData: any[], updateType: 'setData' | 'update' = 'setData') => {
      ...
      try {
          ...
          console.warn("Indicator update skipped", config.type);
      }
  };
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      const priceScaleMargins = hasOscillator ? { top: 0.1, bottom: 0.35 } : { top: 0.1, bottom: 0.1 };
      ...
      indicators.forEach(ind => {
        if (["SMA", "EMA", "BB"].includes(ind.type)) {
           if (ind.type === "BB") {
            ...
            chart!.priceScale(scaleId).applyOptions({ scaleMargins: { top: 0.75, bottom: 0.05 } });
              ...
              if (data) {
                  ...
                  indicatorSeriesRef.current.forEach((ind) => {
                      if (ind.type === 'BB') {
                          ...
                          if (m) tooltipContent += `<div class="mt-1 pt-1 text-[10px] text-muted-foreground flex justify-between items-center"><span style="color: ${ind.config.color}">BB(${ind.config.period})</span><span class="font-mono">${m.value.toFixed(2)}</span></div>`;
                      } else if (ind.type === 'MACD') {
                          const m = ind.macd ? param.seriesData.get(ind.macd) as any : null;
                          const s = ind.signal ? param.seriesData.get(ind.signal) as any : null;
                          if (m && s) tooltipContent += `<div class="mt-1 pt-1 text-[10px] text-muted-foreground"><div class="flex justify-between"><span class="text-blue-500">MACD</span><span class="font-mono">${m.value.toFixed(2)}</span></div><div class="flex justify-between"><span class="text-orange-500">Sig</span><span class="font-mono">${s.value.toFixed(2)}</span></div></div>`;
                      } else {
                          const val = param.seriesData.get(ind.series) as any;
                          if (val) tooltipContent += `<div class="mt-1 pt-1 text-[10px] text-muted-foreground flex justify-between items-center"><span style="color: ${ind.config.color}">${ind.type} ${ind.config.period}</span><span class="font-mono">${val.value.toFixed(2)}</span></div>`;
                      }
                  });
  ...
  }, [symbol, height, showVolume, indicators]);
  ...
  useEffect(() => {
    const handleQuote = (quote: Quote) => {
        ...
        try {
            ...
            // Silently fail real-time updates if series are disposing
        }
    };
  ...
  return (
    <div className="relative w-full group overflow-hidden" style={{ minHeight: height }}>
